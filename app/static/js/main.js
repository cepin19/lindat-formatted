...
Upload.prototype.doUpload = function () {
    ...
    formData.append("source", $("#source option:selected").val())
    formData.append("target", $("#target option:selected").val())
    formData.append("url", get_action_url())
...
//                    var blob = new Blob([data], { type: "application/octetstream" });
                    ...
                    if (isIE) {
                        window.navigator.msSaveBlob(blob, this.file);
                    } else {
                        var url = window.URL || window.webkitURL;
                        //link = url.createObjectURL(blob);
                        link=data;
                        var a = $("<a />");
                        a.attr("download", this.file);
                        a.attr("href", link);
                        $("body").append(a);
                        a[0].click();
                        $("body").remove(a);
